...
[Authorize(Roles = "Recepcionistas")]
public class RecepcionistasController : BaseApiController
{
    ...
    public async Task<ActionResult<IEnumerable<RecepcionistaDto>>> GetAll(CancellationToken ct)
    {
        try
        {
            var recepcionistas = await _service.GetAllAsync(ct);

            if (recepcionistas == null || !recepcionistas.Any())
                return NotFound(new { message = "No se encontraron recepcionistas registrados." });

            var dto = _mapper.Map<IEnumerable<RecepcionistaDto>>(recepcionistas);
            return Ok(dto);
        }
    ...
    public async Task<ActionResult<RecepcionistaDto>> GetById(int id, CancellationToken ct)
    {
        try
        {
            ...
            var recepcionista = await _service.GetByIdAsync(new IdVO(id), ct);

            if (recepcionista is null)
                return NotFound(new { message = $"No se encontró un recepcionista con ID {id}." });

            return Ok(_mapper.Map<RecepcionistaDto>(recepcionista));
        }
    ...
    public async Task<IActionResult> Create([FromBody] CreateRecepcionistaDto body, CancellationToken ct)
    {
        try
        {
            var recepcionista = _mapper.Map<Recepcionista>(body);
            var id = await _service.AddAsync(recepcionista, ct);

            var dto = _mapper.Map<RecepcionistaDto>(recepcionista);
