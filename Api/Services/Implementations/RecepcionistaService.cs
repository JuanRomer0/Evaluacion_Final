...
public class RecepcionistaService : IRecepcionistaService
{
    ...
    {
        if (await _unitOfWork.Recep.ExistsByNombreAsync(recepcionista.Nombre, ct))
            throw new Exception($"Ya existe un recepcionista con el nombre '{recepcionista.Nombre.Value}'");

        var result = await _unitOfWork.Recep.AddAsync(recepcionista, ct);
    ...
    {
        var existing = await _unitOfWork.Recep.GetByIdAsync(recepcionista.Id, ct);
        if (existing == null)
            return false;

        // Validar duplicados
        if (existing.Nombre.Value != recepcionista.Nombre.Value &&
            await _unitOfWork.Recep.ExistsByNombreAsync(recepcionista.Nombre, ct))
        {
            throw new Exception($"Ya existe un recepcionista con el nombre '{recepcionista.Nombre.Value}'");
        }

        var updated = await _unitOfWork.Recep.UpdateAsync(recepcionista, ct);
        if (updated)
    ...
    {
        var existing = await _unitOfWork.Recep.GetByIdAsync(id, ct);
        if (existing == null)
            return false;

        var deleted = await _unitOfWork.Recep.DeleteAsync(id, ct);
        if (deleted)
            await _unitOfWork.SaveChanges(ct);
    ...
    public async Task<bool> ExistsByNombreAsync(NombreVO nombre, CancellationToken ct = default)
        => await _unitOfWork.Recep.ExistsByNombreAsync(nombre, ct);
}
